...
SELECT Examen AS NotaExamen FROM `w3schools-sql`.Students 
WHERE Examen > 7
ORDER BY Examen ASC;

-- Alias con espacios -> '...' o "..."
SELECT Examen AS "Nota del Examen" FROM `w3schools-sql`.Students
...
FROM `w3schools-sql`.Products AS p, `w3schools-sql`.Customers AS c
ORDER BY Direccion;
